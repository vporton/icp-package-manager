...
function ChooseVersion2(props: {
    ...
    const [versionUpgradePrices, setVersionUpgradePrices] = useState<Map<string, bigint> | undefined>();
    ...
    return (
        <>
            ...
            {props.oldInstallation === undefined && chosenVersion !== undefined && versionPrices !== undefined && versionPrices.get(chosenVersion) !== undefined &&
                <p>Price: {Number(versionPrices.get(chosenVersion)!) / 10**12}T cycles from the battery.</p>}
            {props.oldInstallation !== undefined && chosenVersion !== undefined && versionUpgradePrices !== undefined && versionUpgradePrices.get(chosenVersion) !== undefined &&
                <p>Upgrade price: {Number(versionUpgradePrices.get(chosenVersion)!) / 10**12}T cycles from the battery.</p>}
